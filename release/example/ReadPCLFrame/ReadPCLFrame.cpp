...
static class ReceivedPCLFrameSink : public meere::sensor::sink
 , public meere::sensor::prepared_listener
{
public:
	virtual std::string name() const {
		return std::string("ReceivedPCLFrameSink");
	}
	...
	virtual void onCubeEyeFrameList(const meere::sensor::ptr_source source , const meere::sensor::sptr_frame_list& frames) {
		...
		for (auto it : (*frames)) {
			...
			// PointCloud frame
			if (it->frameType() == meere::sensor::CubeEyeFrame::FrameType_PointCloud) {
...
public:
	ReceivedPCLFrameSink() = default;
	virtual ~ReceivedPCLFrameSink() = default;
} _ReceivedPCLFrameSink;
...
int main(int argc, char* argv[])
{
	...
	meere::sensor::add_prepared_listener(&_ReceivedPCLFrameSink);

	// create ToF camera
	meere::sensor::sptr_camera _camera = meere::sensor::create_camera(_source_list->at(_selected_source));
	if (nullptr != _camera) {
		_camera->addSink(&_ReceivedPCLFrameSink);
		...
		// set wanted frame type : PCL 
		int _wantedFrame = meere::sensor::CubeEyeFrame::FrameType_PointCloud;
